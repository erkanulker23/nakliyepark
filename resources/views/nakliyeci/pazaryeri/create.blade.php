...
@section('title', 'Pazaryeri ilanı ekle')
@section('page_heading', 'Pazaryeri ilanı ekle')
@section('page_subtitle', 'Satılık veya kiralık araç ilanı')

@section('content')
<div class="max-w-2xl">
    <div class="admin-card p-6">
        <form method="POST" action="{{ route('nakliyeci.pazaryeri.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div class="admin-form-group">
                <label class="admin-label">İlan başlığı *</label>
                <input type="text" name="title" value="{{ old('title') }}" class="admin-input" placeholder="Örn: 2020 model Mercedes Actros kamyon" required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="admin-form-group">
                    ...
                    <input type="number" name="price" value="{{ old('price') }}" class="admin-input" placeholder="Boş bırakılabilir" min="0" step="0.01">
                ...
                <div class="admin-form-group">
                    ...
                    <input type="number" name="year" value="{{ old('year') }}" class="admin-input" placeholder="Örn: 2020" min="1900" max="{{ date('Y') + 1 }}">
                    @error('year')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
            </div>
            <div class="admin-form-group">
                <label class="admin-label">Şehir</label>
                <input type="text" name="city" value="{{ old('city') }}" class="admin-input" placeholder="Örn: İstanbul">
                @error('city')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
            </div>
            <div class="admin-form-group">
                <label class="admin-label">Açıklama</label>
                <textarea name="description" class="admin-input" rows="4" placeholder="Aracın durumu, özellikleri, iletişim notları...">{{ old('description') }}</textarea>
            ...
            <div class="flex flex-wrap gap-3">
                <button type="submit" class="admin-btn-primary">İlanı yayınla</button>

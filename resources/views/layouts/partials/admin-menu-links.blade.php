{{-- Tüm admin menü linkleri: hem mobil drawer hem masaüstü Menü flyout için aynı liste --}}
@php
    $linkClass = 'flex items-center gap-3 px-3 py-3 rounded-xl text-[var(--panel-text)] hover:bg-[var(--panel-primary-soft)] hover:text-[var(--panel-primary)]';
    $activeClass = 'bg-[var(--panel-primary-soft)] text-[var(--panel-primary)] font-semibold';
@endphp
...
<a href="{{ route('admin.users.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.users.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
    Kullanıcılar
</a>
<a href="{{ route('admin.musteriler.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.musteriler.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
    Müşteriler
</a>
<a href="{{ route('admin.companies.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.companies.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.ihaleler.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.ihaleler.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.teklifler.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.teklifler.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
    Teklifler
</a>
<a href="{{ route('admin.yuk-ilanlari.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.yuk-ilanlari.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.reklam-alanlari.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.reklam-alanlari.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.reviews.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.reviews.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
    Değerlendirmeler
</a>
<a href="{{ route('admin.disputes.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.disputes.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
    Uyuşmazlıklar
</a>
<a href="{{ route('admin.blog.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.blog.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.blog-categories.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.blog-categories.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.faq.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.faq.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.homepage-editor.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.homepage-editor.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
    Anasayfa düzenleyici
</a>
<a href="{{ route('admin.sponsors.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.sponsors.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
    Sponsorlar
</a>
<a href="{{ route('admin.consent-logs.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.consent-logs.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
    KVKK Rıza Logları
</a>
<a href="{{ route('admin.blocklist.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.blocklist.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.sitemap.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.sitemap.*') ? $activeClass : '' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>
    Sitemap
</a>
<a href="{{ route('admin.settings.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.settings.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.site-contact-messages.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.site-contact-messages.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.notifications.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.notifications.*') ? $activeClass : '' }}">
...
<a href="{{ route('admin.profile.edit') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.profile.*') ? $activeClass : '' }}">

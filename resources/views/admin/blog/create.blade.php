    ...
    <div class="admin-card p-6">
        <form method="POST" action="{{ route('admin.blog.store') }}" enctype="multipart/form-data" class="space-y-5" id="blog-form">
            ...
            <div class="admin-form-group">
                ...
                <textarea name="ai_prompt" id="blog-ai-prompt" rows="3" class="admin-input" placeholder="Yapay zeka ile oluştururken veya sonradan referans için kullanılacak prompt veya talimat notları (isteğe bağlı)">{{ old('ai_prompt') }}</textarea>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">AI ile oluşturma talimatları veya makale notları. Sadece admin panelinde görünür, yayında gösterilmez.</p>
            ...
            <div class="admin-form-group">
                ...
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Başlık, kalın/italik/altı çizili, liste, link, alıntı ve kod. İçerik frontend’de birebir aynı görünecektir. Reklam göstermek istediğiniz yere <code class="px-1 py-0.5 rounded bg-slate-200 dark:bg-slate-600">[reklam]</code> yazın (Reklam Alanları'nda &quot;Blog yazısı içeriği&quot; konumunda tanımlı reklam gösterilir).</p>
            </div>
            <div class="border-t border-slate-200 pt-5">

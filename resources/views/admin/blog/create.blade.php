    ...
    <div class="admin-card p-6">
        <form method="POST" action="{{ route('admin.blog.store') }}" class="space-y-5">
            ...
            <div class="admin-form-group">
                ...
                <textarea name="content" rows="16" required class="admin-input font-mono text-sm" placeholder="HTML veya düz metin içerik">{{ old('content') }}</textarea>
                @error('content')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                <p class="mt-1 text-xs text-slate-500">HTML etiketleri kullanabilirsiniz (örn. &lt;p&gt;, &lt;strong&gt;, &lt;a href="..."&gt;).</p>
            </div>
